...
function getResetGain(layer, useType = null) {
	if(layer != "p1"){
		...
		if((player.points).equals(new Decimal(0))){
			return new Decimal(0)
		}else{
			let tg = ((player.points).div(player.p.vapl).log(10).add(25))
			if(tg.gte(0)){
				tg = ((tg.pow(new Decimal(0.8).mul(upgradeEffect("p1",16)))).floor().max(0))
				return tg
			}
			return new Decimal(0)
		}
...
function getNextAt(layer, canMax=false, useType = null) {
	if(layer != "p1"){
		...
		return ((new Decimal(10).pow((getResetGain("p1")).add(1).pow(new Decimal(1).div(new Decimal(0.8).mul(upgradeEffect("p1",16)))).sub(25))).max(new Decimal(10).pow(-24.9999999999999))).mul(player.p.vapl)
	}
}
...
{	
	if (layers[layer].canReset!== undefined)
		...
		return tmp[layer].baseAmount.gte(tmp[layer].requires.mul(player.p.vapl))
...
VERSION.withoutName = "alpha" + VERSION.num + (VERSION.pre ? " Pre-Release " + VERSION.pre : VERSION.pre ? " Beta " + VERSION.beta : "")
...
function hardReset(resetOptions) {
	if (!confirm("诶？你确定要硬重置嘛？这会删除这个存档的！如果确定的话……点“确定”按钮就好啦~")) return
	player = null
