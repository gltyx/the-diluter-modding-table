function prestigeButtonText(layer) {
	...
	if (tmp[layer].type == "normal")
		return `${player[layer].points.lt(1e3) ? (tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : (i18n("重置以获得 ", 'Reset for ', false))) : ""}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource} ${tmp[layer].resetGain.lt(100) && player[layer].points.lt(1e3) ? `<br><br>`+(i18n("下一个在:", "Neat at:", false))+` ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : formatSmall(tmp[layer].nextAt))} ${tmp[layer].baseResource == "points" ? "稀释点" : tmp[layer].baseResource}` : ""}`
	if (tmp[layer].type == "static")
		return `${tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : (i18n("重置以获得 ", 'Reset for ', false))}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource}<br><br>${player[layer].points.lt(30) ? (tmp[layer].baseAmount.gte(tmp[layer].nextAt) && (tmp[layer].canBuyMax !== undefined) && tmp[layer].canBuyMax ? (i18n("下一个:<br>", "Next:", false)) : (i18n("需求:<br>", "Req:", false))) : ""} ${formatWhole(tmp[layer].baseAmount)}/${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAtDisp) : formatSmall(tmp[layer].nextAtDisp))} ${tmp[layer].baseResource == "points" ? "<br>稀释点" : "<br>"+tmp[layer].baseResource}		
		`
...
function challengeButtonText(layer, id) {
    return (player[layer].activeChallenge==(id)?(canCompleteChallenge(layer, id)?"完成并退出":"提前退出"):(hasChallenge(layer, id)?"已完成":"开始"))
